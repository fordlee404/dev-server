...
var htmlIncludeReplace = require("html-include-replace");
      ...
      if(options.contentBase !== false) {
        ...
        if(typeof contentBase === "object") {
          ...
          app.get("*", function(req, res, next){
            var _path = req.path, config = {};

            if(options.includeReplace){
              config = options.includeReplace;
            }

            if(path.extname(_path)===''||path.extname(_path)==='.html'){
              if(path.extname(_path)===''){
                _path += 'index.html'
              }
              _path = contentBase + _path;

              try{
                res.setHeader("Content-Type", "text/html");
                res.write(htmlIncludeReplace(_path, config));
                res.end();
              }catch(err){
                console.log('html-include-replace error: '+err);
                next();
              }
            }else{
              next();
            }
          }.bind(this), express.static(contentBase), serveIndex(contentBase));
        }
      }
    }.bind(this),
